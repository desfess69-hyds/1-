...
- [x] 5-1. `com.hyds.weekly-trends.plist` 생성 (매주 월 09:30, daily-monitor와 같은 시각이 아니어야 하므로 09:45로 조정)
- [x] 5-2. `scout_trends.py --weekly` 모드 — 본인이 입력한 관심 키워드 5개 자동 처리
- [x] 5-3. `data/trend_keywords.json` — 본인이 직접 관리하는 관심 키워드 풀
- [x] 5-4. 텔레그램 발송: 트렌드 5개 + CapCut 검색어 + 추천 시간 카드 형식
- [x] 5-5. launchd 등록 가이드 README 추가
- [x] 5-6. git 커밋: "HYDS: 주간 트렌드 자동 보고 자동화"
...
- [x] 6-2. **작업 기억**: 모든 sub-agent에 "PLAN/CONTEXT/CHECKLIST 참조" 지침 명시 (미디어 5명 적용; 수련회 4명은 미디어 문서 무관해 제외)
- [x] 6-3. **품질 검사**:
...
- [x] 7-1. 채팅 명령 "@미디어 평택교회 수련회 30초 릴스 짜줘" — 흐름 확인
- [x] 7-2. 결과물 9개 파일 확인 → Vrew에 vrew_script.txt 붙여넣기 테스트
- [x] 7-3. CapCut에서 trend_brief.md의 검색어로 템플릿 찾기 테스트
- [x] 7-4. 매주 월 09:45 트렌드 보고 텔레그램 도착 확인
- [x] 7-5. 발견된 어색함·누락 → `directives/learnings.md`에 한 줄 추가 (self-anneal)
- [x] 7-6. git 커밋: "HYDS: 미디어팀 Phase 1 운영 학습 반영"

---

## 단계 8 — Phase 1 종결 보고

- [x] 8-1. 미디어팀 운영 한 달 후 → 부장이 "주간 종합 보고"에 미디어팀 지표 포함
- [x] 8-2. Remotion·YouTube API 등 Phase 2 검토 여부 결정
- [x] 8-3. PLAN.md 상태를 "✅ Phase 1 완료, Phase 2 검토" 로 변경
...
총 항목 수(체크박스): **51개**
완료: **51** — ✅ **Phase 1 전체 완주 (단계 0~8)**
진척률: **100%** (51개 중 51)
다음: Phase 2 검토 → `docs/PHASE2_CANDIDATES.md` 참고
